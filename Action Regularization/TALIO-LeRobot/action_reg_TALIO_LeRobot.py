...
class TALIOCompressedLeRobotDataset(Dataset):
    """
    Wraps LeRobotDataset to cleanly inject TALIO temporal compression boundaries.
    Applies action regularization across discovered sub-action segments using an 
    offline-computed json mapping prior to tensor normalization steps.
    """
    def __init__(self, base_dataset: Dataset, talio_json_path: str):
        self.base_dataset = base_dataset
        
        # Load pre-computed boundaries
        if not os.path.exists(talio_json_path):
            raise FileNotFoundError(f"TALIO registry file not found at: {talio_json_path}")
            
        with open(talio_json_path, "r") as f:
            self.sub_skills_registry = json.load(f)
            
        logging.info(f"Successfully loaded TALIO regularization registry from {talio_json_path}")

        # Expose foundational attributes to ensure downstream script compatibility
        self.config = getattr(base_dataset, 'config', None)
        self.arm2stats_dict = getattr(base_dataset, 'arm2stats_dict', {})
        self.max_action_dim = getattr(base_dataset, 'max_action_dim', None)
        self.max_state_dim = getattr(base_dataset, 'max_state_dim', None)
        self.max_views = getattr(base_dataset, 'max_views', None)

    def __len__(self) -> int:
        return len(self.base_dataset)

    def __getitem__(self, idx: int) -> dict:
        # 1. Fetch path to target file tracker
        cache_filepath = self.base_dataset.data[idx]
        ...
        try:
            ...
            logging.info(f"Cannot load cache file {cache_filepath}: {str(e)}")
            return self.base_dataset[random.randint(0, len(self.base_dataset) - 1)]

        # 2. Extract context mappings for segment lookup
        # LeRobot tracks files inside structured parent folders. We isolate the trajectory ID 
        # using the file path configuration (e.g., using the parquet parent or stem name)
        episode_id = Path(cache_filepath).parent.name  # Extracts unique trial/trajectory name
        
        # Extract individual window configurations
        filename_stem = Path(cache_filepath).stem      # Format: "startIdx_endIdx"
        try:
            frame_idx = int(filename_stem.split('_')[0])
        except (ValueError, IndexError):
            frame_idx = -1

        # 3. Apply TALIO temporal action regularization directly on raw data matrix
        if episode_id in self.sub_skills_registry and frame_idx != -1:
            ...
            for seg in segments:
                if seg["start"] <= frame_idx < seg["end"]:
                    # Isolate action window arrays before normalization passes
                    raw_actions = item["action"] # List of arrays/lists
                    
                    if isinstance(raw_actions, list):
                        raw_actions_np = np.array(raw_actions)
                    else:
                        raw_actions_np = raw_actions
                    
                    # Compute mean action signature over the segment horizon
                    mean_action = np.mean(raw_actions_np, axis=0)
                    
                    # Overwrite action tracking matrix with uniform spatial goals
                    regularized_actions = np.tile(mean_action, (len(raw_actions_np), 1))
                    item["action"] = [row for row in regularized_actions]
                    break

        # 4. Forward modified dictionary through standard LeRobot parsing methods
        arm_key = item["arm_key"]
        dataset_key = item["dataset_key"]
        embodiment_id = self.base_dataset.arm_to_embodiment_id[arm_key]

        try:
            frames = self.base_dataset._load_video_frame(item["video_paths"], item["timestamp"])
        except Exception as e:
            logging.info(f"Skipping sample that cannot decode video {cache_filepath}: {e}")
            return self.base_dataset[random.randint(0, len(self.base_dataset) - 1)]

        images = frames

        if self.base_dataset.use_augmentation:
            images = [
                self.base_dataset.aug_transform(img) if random.random() < 0.5 
                else self.base_dataset.basic_transform(img)
                for img in images
            ]
        else:
            images = [self.base_dataset.basic_transform(img) for img in images]

        num_real_views = len(images)
        image_mask = torch.zeros(self.base_dataset.max_views, dtype=torch.bool)
        image_mask[:num_real_views] = True 

        while len(images) < self.base_dataset.max_views:
        ...
        norm_stats = self.base_dataset.arm2stats_dict[arm_key]
        ...
        state_padded, state_mask = self.base_dataset._pad_tensor(state, self.base_dataset.max_state_dim)

        # Process Regularized Action Tensors
        action = torch.from_numpy(np.stack(item["action"])).float()
        action_min = torch.tensor(norm_stats["action"]["min"], dtype=torch.float32, device=action.device)
        action_max = torch.tensor(norm_stats["action"]["max"], dtype=torch.float32, device=action.device)
        
        action = 2 * (action - action_min.unsqueeze(0)) / (action_max.unsqueeze(0) - action_min.unsqueeze(0) + 1e-8) - 1
        action = torch.clamp(action, -1.0, 1.0)

        action_padded, action_mask = self.base_dataset._pad_tensor(action, self.base_dataset.max_action_dim)
